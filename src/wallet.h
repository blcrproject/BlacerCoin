...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BLCR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBLCR send
enum ZerocoinSpendStatus {
    ZBLCR_SPEND_OKAY = 0,                            // No error
    ZBLCR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBLCR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBLCR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBLCR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBLCR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBLCR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBLCR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBLCR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBLCR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBLCR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBLCR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBLCR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBLCR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBLCR_SPENT_USED_ZBLCR = 14,                      // Coin has already been spend
    ZBLCR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBLCR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBLCROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzBLCRWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBLCRTracker> zpivTracker;
    ...
    void setZWallet(CzBLCRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzBLCRTracker>(new CzBLCRTracker(strWalletFile));
    }

    CzBLCRWallet* getZWallet() { return zwalletMain; }
    ...
    /** zBLCR reset */
    boost::signals2::signal<void()> NotifyzBLCRReset;
