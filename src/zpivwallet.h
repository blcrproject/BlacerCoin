...
#ifndef BlacerCoin_ZBLCRWALLET_H
#define BlacerCoin_ZBLCRWALLET_H
...
public:
    CzBLCRWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBLCR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBLCR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BlacerCoin_ZBLCRWALLET_H
