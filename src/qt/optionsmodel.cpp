...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BLCR);
    ...
    if (!settings.contains("nAnonymizeBlacercoinAmount"))
        settings.setValue("nAnonymizeBlacercoinAmount", 1000);

    nAnonymizeBlacercoinAmount = settings.value("nAnonymizeBlacercoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBlacercoinAmount"))
        SoftSetArg("-anonymizeblacercoinamount", settings.value("nAnonymizeBlacercoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in blacercoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBlacercoinAmount:
            return QVariant(nAnonymizeBlacercoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBlacercoinAmount:
            nAnonymizeBlacercoinAmount = value.toInt();
            settings.setValue("nAnonymizeBlacercoinAmount", nAnonymizeBlacercoinAmount);
            emit anonymizeBlacercoinAmountChanged(nAnonymizeBlacercoinAmount);
            break;
        case CoinControlFeatures:
