...
public:
    ...
    bool WriteZBLCRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBLCRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBLCRSeed_deprecated(uint256& seed);
    bool EraseZBLCRSeed();
    bool EraseZBLCRSeed_deprecated();

    bool WriteZBLCRCount(const uint32_t& nCount);
    bool ReadZBLCRCount(uint32_t& nCount);
