...
CzBLCRTracker::CzBLCRTracker(std::string strWalletFile)
{
...
CzBLCRTracker::~CzBLCRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBLCRTracker::Init()
{
...
bool CzBLCRTracker::Archive(CMintMeta& meta)
{
...
bool CzBLCRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBLCRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBLCRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBLCRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBLCRTracker::GetSerialHashes()
{
...
CAmount CzBLCRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBLCRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBLCRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBLCRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBLCRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBLCRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBLCRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBLCRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBLCRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBLCRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBLCRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBLCRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBLCRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBLCRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBLCRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBLCRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBLCRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
