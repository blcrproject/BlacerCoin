...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("blacercoin-wallet");
...
bool CWalletDB::WriteZBLCRSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZBLCRSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZBLCRSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZBLCRSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZBLCRSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZBLCRSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZBLCRCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZBLCRCount(uint32_t& nCount)
{
