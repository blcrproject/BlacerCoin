...
CzBLCRWallet::CzBLCRWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBLCRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBLCRSeed_deprecated()) {
                    LogPrintf("%s: Updated zBLCR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBLCRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBLCRCount(nCountLastUsed);
    else if (!walletdb.ReadZBLCRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBLCRWallet::Lock()
{
    seedMaster = 0;
}

void CzBLCRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBLCRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBLCR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBLCRWallet::LoadMintPoolFromDB()
{
...
void CzBLCRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBLCRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBLCRWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBLCRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBLCR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBLCRWallet::SeedToZBLCR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBLCRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBLCRWallet::GenerateDeterministicZBLCR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBLCRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBLCRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
